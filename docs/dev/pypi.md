...
> **Status**: The PyPI publishing workflow is currently **disabled**. The workflow file exists as `.github/workflows/pypi.yaml.old` and is not active. To re-enable it, rename the file to `pypi.yaml` and configure OIDC trusted publishing on PyPI (see [Re-enabling PyPI Publishing](#re-enabling-pypi-publishing) below).
>
> The active release automation builds a Windows executable (`rbcopy.exe`) — see [github.md](./github.md) for details.

## Overview

When active, the PyPI workflow will:

- Build source distributions and wheels on every push and pull request
- Validate that the package can be built successfully
- Publish to PyPI when version tags are pushed
- Use trusted publishing via OIDC (no API tokens required)

## Re-enabling PyPI Publishing

1. Rename the workflow file:

   ```bash
   git mv .github/workflows/pypi.yaml.old .github/workflows/pypi.yaml
   git commit -m "chore: re-enable PyPI publishing workflow"
   ```

2. Configure OIDC trusted publishing on PyPI:
   - Log in to [pypi.org](https://pypi.org)
   - Navigate to your project → Publishing → Add a publisher
   - Set the GitHub repository, workflow file name (`pypi.yaml`), and environment (`pypi`)
...
# Verify installation
python -c "import rbcopy; print(rbcopy.__version__)"
...
The PyPI workflow (`.github/workflows/pypi.yaml.old` — currently disabled) is configured to run on:
...
1. Go to your project page: `https://pypi.org/project/rbcopy/`
...
```bash
# View on PyPI
open https://pypi.org/project/rbcopy/
