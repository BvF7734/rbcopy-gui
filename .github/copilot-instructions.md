...
* Application state and presets are managed via Pydantic models in `rbcopy/presets.py`. Do not use Tkinter variables for persistent file state.

### Threading and Async

* Never interact with Tkinter widgets (`tk.` or `ttk.`) directly from inside an asyncio coroutine or background thread. Always use a thread-safe `queue.Queue` or `widget.after(0, ...)` to pass data back to the main UI thread.
